...
class Operator : public BaseRuntimeStatWriter {
 public:
  ...
  /// Returns true if this operator accepts dynamic filters on any output
  /// channel.
  virtual bool canAddDynamicFilter() const {
    return false;
  }

  /// Returns whether this operator accepts a filter on an output channel.
  virtual bool canAddDynamicFilterOnChannel(column_index_t /*channel*/) const {
    return canAddDynamicFilter();
  }

  /// Adds pending filters dynamically generated by a downstream operator.
  /// Called only if canAddDynamicFilterOnChannel() returns true. The driver
  /// already holds a shared lock on PushdownFilters.
